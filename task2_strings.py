...
#a) Extract the name of the 5th game in the dataset (index 4) and store it in a variable called game_name. Use string indexing to extract just the word "Pokemon" from it and print the result.

game_name = (video_game_sales[4][1]) 
print(game_name[:7]) 

 #b) The game names below have extra whitespace and inconsistent capitalization. Clean them by stripping whitespace and converting to lowercase. Print each cleaned name.

for name in messy_names:
    clean_names = name.strip().lower()
    print(clean_names)

#c) Use an f-string to print a formatted summary of the #1 game in the format:

print(f"#1 Best Seller: {video_game_sales[0][NAME]} ({video_game_sales[0][YEAR]}) - ${video_game_sales[0][GLOBAL_SALES]}M global sales")
