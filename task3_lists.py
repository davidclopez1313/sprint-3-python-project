...
#a), start with an empty list and use a for loop with .append() to add each game's name.

game_names = []

for name in video_game_sales:
    game_names.append(name[1])
    
print(game_names)

#b)use the .append() method to add the new game row to video_game_sales.
...
#c)create a tuple using parentheses () — remember that tuples are immutable, which is appropriate for metadata that shouldn't change.

number_of_games = 21 
number_of_columns = 10
name_of_dataset = 'Video Game Sales'

dataset_info = (number_of_games, number_of_columns, name_of_dataset)

print(dataset_info)

#Tuple is more appropriate than a list for this data because tuple is fixed/immutable and this metadata should not change because we have already done the calculations from the previous tasks
