...
#a) Store the total number of games in the dataset in a variable called total_games. Print its value.
...
for i in video_game_sales:
    total_sales = total_sales + i[GLOBAL_SALES]
    
#b) Calculate the average global sales across all 20 games. Store the result in a variable called avg_global_sales and print it with a descriptive message.

avg_global_sales = total_sales / total_games
print(f"Average global sales: {avg_global_sales}")

#c) The game at index 0 (Wii Sports) has the highest global sales. Calculate what percentage of the total global sales (sum of all 20 games) it represents. Store the result in top_game_share and print it.
